...
function drawGoldenTriangle(pos, height, rotation, pivot) {
  ...
  const pointA = rotatePoint(pos, rotation, [pos[0], pos[1] - height]);
  // sharpest angle
...
const startingPoint = (a, b, c) => {
  ...
  const d = [cd * (b[0] - c[0]) / bc + c[0], cd * (b[1] - c[1]) / bc + c[1]];
  const e = [(a[0] + c[0]) / 2, (a[1] + c[1]) / 2];
  const f = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2];
...
function loop() {
  ...
  const pointA = [triangleLocation[0], triangleLocation[1] - triangleHeight];
  // sharpest angle
...
},{"./math":"wONcP"}],"wONcP":[function(require,module,exports) {
...
},{"decimal.js":"3k62q"}],"3k62q":[function(require,module,exports) {
var define;
;
(function (globalScope) {
  ...
  */
  P.minus = P.sub = function (y) {
    ...
    if (!x.d || !y.d) {
      ...
      if (!x.s || !y.s) y = new Ctor(NaN); else // Return y negated if x is finite and y is ±Infinity.
      if (x.d) y.s = -y.s; else // Return x if y is finite and x is ±Infinity.
    ...
    if (!xd[0] || !yd[0]) {
      ...
      if (yd[0]) y.s = -y.s; else // Return x if y is zero and x is non-zero.
      if (xd[0]) y = new Ctor(x); else // Return zero if both are zero.
  ...
  */
  P.plus = P.add = function (y) {
    ...
    if (!x.d || !y.d) {
      // Return NaN if either is NaN.
      if (!x.s || !y.s) y = new Ctor(NaN); else // Return x if y is finite and x is ±Infinity.
...
},{}]},["63iPG","4ThtM"], "4ThtM", "parcelRequireb8e3")
